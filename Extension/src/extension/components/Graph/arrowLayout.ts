  ...
  if (nodeL.x  == nodeR.x){
    ...
    if (nodeL.y < nodeR.y) {
      // form position 1 to 2
        return {
            from: {
            x2: nodeL.x + getWidth(nodeL.node.isCall || nodeL.node.isSink || false)/2 + padding - gridRect.x,
            y2: nodeL.y + getHeight(nodeL.node.isCall || nodeL.node.isSink || false) - gridRect.y
          },
          to: {
            x1: nodeR.x + getWidth(nodeR.node.isSource || false) + 2 * padding - gridRect.x,
            y1: nodeR.y + getHeight(nodeR.node.isCall || nodeR.node.isSink || false)/2 - gridRect.y
      ...
      return{
        from: {
          x2: nodeL.x + getWidth(nodeL.node.isSource || false)/2 - gridRect.x,
          y2: nodeL.y - padding - gridRect.y
        },
        to: {
          x1: nodeR.x + getWidth(nodeR.node.isSource || false) + 2 * padding - gridRect.x,
          y1: nodeR.y + getHeight(nodeR.node.isCall || nodeR.node.isSink || false)/2 - gridRect.y
